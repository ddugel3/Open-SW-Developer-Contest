...
//                        FindPath_Car();
    ...
    private void readNextDetailMessage() {
        if (currentDetailMessageIndex < message_detail_string.length) {
            String message = message_detail_string[currentDetailMessageIndex];
            textToSpeech.speak(message, TextToSpeech.QUEUE_FLUSH, null, null);
            currentDetailMessageIndex++;
        }
    }
    // 모든 상세 메시지 읽기
    private void readAllDetailMessages() {
        ...
        for (int i = 0; i < pl; i++) {
            ...
            }, i * delay); // 딜레이 시간을 현재 인덱스에 따라 증가시킴
        }
    }
